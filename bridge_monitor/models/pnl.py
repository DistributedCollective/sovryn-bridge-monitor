...
class ProfitCalculation(Base):
    ...
    config_chain = Column(
        Text, nullable=False
    )  # denormalized. rsk_mainnet/rsk_testnet but never bitcoin. used for filtering
    timestamp = Column(
        TZDateTime, nullable=False
    )  # unique timestamp for a transfer, to be used for filtering

    transactions = relationship(
        "PnLTransaction",
        back_populates="profit_calculation",
        order_by="PnLTransaction.timestamp",
    )

    volume_btc = Column(
        Numeric, nullable=False
    )  # total amount transferred, including fees
    gross_profit_btc = Column(Numeric, nullable=False)  # fee paid to us (gross profit)
    cost_btc = Column(
        Numeric, nullable=False
    )  # cost in btc, decimal value. denormalized because this can be calculated from transfers
...
class PnLTransaction(Base):
    ...
    profit_calculation_id = Column(
        Integer, ForeignKey("pnl_calculation.id"), nullable=False
    )
    profit_calculation = relationship(
        "ProfitCalculation", back_populates="transactions"
    )

    cost_btc = Column(Numeric, nullable=False)  # cost in btc, decimal value

    transaction_chain = Column(
        Text, nullable=False
    )  # can be rsk_mainnet/rsk_testnet but also "bitcoin"
