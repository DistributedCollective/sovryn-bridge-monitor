...
def update_bidi_fastbtc_transfers(
    ...
    *,
    ...
    if from_block > to_block:
        logger.info("Bidi FastBTC start block %s is larger than bridge end block %s -- skipping",
                    from_block,
                    to_block)
        return
    ...
    logger.info("Found %s BitcoinTransferBatchSending events", len(bitcoin_transfer_batch_sending_events))
    ...
    with transaction_manager:
        ...
        for event in bitcoin_transfer_status_updated_events:
            ...
            if status == TransferStatus.SENDING:
                ...
                transfer.marked_as_sending_transaction_hash = event.transactionHash.hex()
                ...
                transfer.refunded_or_reclaimed_transaction_hash = event.transactionHash.hex()
