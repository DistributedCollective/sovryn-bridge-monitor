...
from .constants import FASTBTC_IN_CONFIGS, FASTBTC_IN_MANAGEDWALLET_ABI, FASTBTC_IN_MULTISIG_ABI
...
def update_fastbtc_in_transfers(
    ...
    *,
    ...
    multisig = web3.eth.contract(
        abi=FASTBTC_IN_MULTISIG_ABI,
        address=config['multisig_address']
    )
    managed_wallet = web3.eth.contract(
        abi=FASTBTC_IN_MANAGEDWALLET_ABI,
        address=config['managedwallet_address']
    )
    ...
    if max_blocks_from_now and to_block - max_blocks_from_now > from_block:
        logger.info("Limiting FastBTC-in start block to %s blocks from now (%s instead of %s)",
                    max_blocks_from_now,
    ...
    if from_block > to_block:
        logger.info("FastBTC-in start block %s is larger than bridge end block %s -- skipping",
                    from_block,
                    to_block)
        return
    ...
    for i, event in enumerate(multisig_events, start=1):
        ...
        if transaction_id is not None and transaction_id not in multisig_transactions_by_tx_id:
            multisig_transactions_by_tx_id[transaction_id] = multisig.functions.transactions(transaction_id).call()
    ...
    with transaction_manager:
        ...
        for event in multisig_events:
            ...
            if event.event not in ('Submission', 'Confirmation', 'Revocation', 'Execution', 'ExecutionFailure'):
            ...
            if destination != managed_wallet.address:
                logger.info("Ignoring fastbtc-in event %s tx %s with destination %s (not ManagedWallet)",
                            event.event,
                            transaction_id,
                            destination)
                continue

            managed_wallet_func, managed_wallet_args = managed_wallet.decode_function_input(data)
            ...
            if managed_wallet_func.function_identifier not in [
                ...
                logger.info("Ignoring fastbtc-in event %s tx %s with function %s",
                            event.event,
                            transaction_id,
            ...
            if event.event == 'Submission':
                ...
                logger.info('ExecutionFailure(%s) at block %s', event.args, block['number'])
