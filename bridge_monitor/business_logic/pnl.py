...
class PnLService:
    ...
    def update_pnl_for_bidi_fastbtc_transfers(self):
        logger.info(
            "Retrieving bidi-fastbtc transfer batches with unprocessed PnL calculations"
        )
        with self._transaction_manager:
            dbsession = self._get_dbsession()

            mined_transfer_batches = (
                dbsession.query(
                    BidirectionalFastBTCTransfer.chain,
                    BidirectionalFastBTCTransfer.marked_as_sending_transaction_hash,
                    func.array_agg(
                        BidirectionalFastBTCTransfer.marked_as_mined_transaction_hash
                    ).label("mined_tx_hashes"),
                    func.array_agg(BidirectionalFastBTCTransfer.bitcoin_tx_id).label(
                        "bitcoin_tx_ids"
                    ),
                ...
                .filter(
                    ...
                    BidirectionalFastBTCTransfer.status
                    == BidiFastBTCTransferStatus.MINED,
                )
                .group_by(
                    BidirectionalFastBTCTransfer.chain,
                    BidirectionalFastBTCTransfer.marked_as_sending_transaction_hash,
        ...
        for i, batch in enumerate(mined_transfer_batches, start=1):
            logger.info(
                "Updating PnL for bidi-fastbtc batch %d/%d",
                i,
                len(mined_transfer_batches),
            )
            try:
                ...
                logger.exception(
                    "Error while processing batch %d/%d", i, len(mined_transfer_batches)
                )
    ...
    def _update_pnl_for_mined_bidi_transfer_batch(self, batch):
        ...
        transfers = (
            ...
            .order_by(BidirectionalFastBTCTransfer.id)
            .all()
        )
        if not len(transfers) == len(transfer_ids):
            raise Exception(
                f"Expected {len(transfer_ids)} transfers, got {len(transfers)}"
            )
        if not all(t.chain == chain for t in transfers):
            raise Exception(f"Expected all transfers to be on chain {chain}")
        if not all(
            t.marked_as_sending_transaction_hash == sending_tx_hash for t in transfers
        ):
            # Sanity check, should never happen
            raise Exception(
                f"Expected all transfers to have sending tx hash {sending_tx_hash}"
            )
        if not all(
            t.marked_as_mined_transaction_hash == mined_tx_hash for t in transfers
        ):
            # Sanity check, should never happen
            raise Exception(
                f"Expected all transfers to have mined tx hash {mined_tx_hash}"
            )
        if not all(t.bitcoin_tx_id == bitcoin_tx_id for t in transfers):
            # Sanity check, should never happen
            raise Exception(
                f"Expected all transfers to have bitcoin tx id {bitcoin_tx_id}"
            )

        sending_tx = self._create_evm_pnl_transaction(
            chain, sending_tx_hash, comment="mark_transfers_as_sending"
        )
        mined_tx = self._create_evm_pnl_transaction(
            chain, mined_tx_hash, comment="mark_transfers_as_mined"
        )
        bitcoin_tx = self._create_bitcoin_pnl_transaction(
            chain, bitcoin_tx_id, comment="bitcoin_tx"
        )
    ...
    def _update_pnl_for_fastbtc_in_transfer(self, transfer_id):
        ...
        if transfer.profit_calculation_id is not None:
            logger.warning(
                "Transfer %s already has a PnL calculation, skipping", transfer_id
            )
            return
        ...
        if execution_tx.transaction_id not in set(
    ...
    def update_pnl_for_bidi_fastbtc_replenisher_transactions(self):
        logger.info(
            "Retrieving bidi-fastbtc replenisher transactions with unprocessed PnL calculations"
        )
        with self._transaction_manager:
            dbsession = self._get_dbsession()
            ids = self._get_unprocessed_object_ids(
            ...
            for i, replenisher_tx_id in enumerate(ids, start=1):
                ...
                replenisher_tx = dbsession.query(
                    BidirectionalFastBTCReplenisherTransaction
                ).get(replenisher_tx_id)
                profit_calculation = ProfitCalculation(
                    ...
                    volume_btc=Decimal(
                        "0"
                    ),  # replenishment amount doesn't count towards volume
                    gross_profit_btc=Decimal(
                        "0"
                    ),  # replenishment fee doesn't count towards profit
    ...
    def _create_evm_pnl_transaction(
        self, chain: str, transaction_hash: str, *, comment=""
    ):
    ...
    def _create_bitcoin_pnl_transaction(
        self, config_chain: str, transaction_id: str, *, comment=""
    ):
    ...
    def _get_evm_block_timestamp(self, web3: Web3, block_identifier) -> datetime:
        ...
        assert isinstance(block_identifier, int) or (
