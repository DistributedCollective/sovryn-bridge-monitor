...
Revision ID: 04c35b0474d1
Revises: 3bb7baafa019
Create Date: 2024-05-08 13:55:30.149607
...
def upgrade():
    op.create_table(
        ...
        sa.CheckConstraint(
            "LOWER(address) = address", name=op.f("ck_rsk_address_lowercase_address")
        ),
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["address_id"],
            ["rsk_address.id"],
            name=op.f("fk_rsk_tx_bookkeeper_address_id_rsk_address"),
        ),
    ...
    op.create_table(
        ...
        sa.Column(
            "trace_json", postgresql.JSONB(astext_type=sa.Text()), nullable=False
        ),
        sa.ForeignKeyConstraint(
            ["address_id"],
            ["rsk_address.id"],
            name=op.f("fk_rsk_tx_info_address_id_rsk_address"),
        ),
