...
@view_config(
    route_name="replenisher", renderer="bridge_monitor:templates/replenisher.jinja2"
)
def replenisher(request):
    ...
    bidi_fastbtc_transactions = (
        dbsession.query(BidirectionalFastBTCReplenisherTransaction)
        .filter(BidirectionalFastBTCReplenisherTransaction.config_chain == chain)
        .order_by(BidirectionalFastBTCReplenisherTransaction.block_timestamp.desc())
