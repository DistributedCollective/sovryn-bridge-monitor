...
from bridge_monitor.models.fastbtc_in import (
    FastBTCInTransfer,
    FastBTCInTransferStatus
)


@view_config(route_name='fastbtc_in', renderer='bridge_monitor:templates/fastbtc_in.jinja2')
def fastbtc_in(request):
    ...
    ordering = [
        FastBTCInTransfer.submission_block_timestamp.desc()
    ]

    transfers = dbsession.query(FastBTCInTransfer).filter(
